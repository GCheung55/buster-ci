...
var Agent = require("buster-ci-agent"),
    ...
    async = require("async"),
    when = require("when"),
    ServerCli = require("./server-cli"),
    TestCli = require("./test-cli");
...
function BusterCi(config) {
    ...
    this._server = ServerCli.create();

    this._testCli = TestCli.create(config);
...
BusterCi.prototype = {
    ...
    run: function (args, cb) {
        ...
        // var testCli = this._testCli
        // var newExitMessage = function(response){
        //     if (response.method === 'exit') {
        //         testCli.removeListener('message', newExitMessage)
        //         cb(exitCode = response.exitCode)
        //     }
        // }
        // testCli.on('message', newExitMessage)
        ...
        async.auto(
            ...
            function (err, results) {
                ...
                closeBrowsers.call(this, function (err) {
                    ...
                    //     if (response.method === 'exit') {
                    //         testCli.removeListener('message', exitMessage)
                    //         cb && cb(response.exitCode)
                    //         process.exit(response.exitCode)
                    //     }
                    // }

                    // testCli.on('message', exitMessage);

                    // this._testCli.send({
                    //     method: 'exit',
                    //     exitCode: exitCode
                    // })
                    this._testCli.exit(cb || function(err, exitCode){
